...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None):
        ...
        if self.embed_positions is not None:
            x = x + self.embed_positions(x_in.transpose(0, 1).contiguous()[:, :, 0]).transpose(0,
        ...
        if x_in_k is not None and x_in_v is not None:
            ...
            if self.embed_positions is not None:
                x_k = x_k + self.embed_positions(x_in_k.transpose(0, 1)[:, :, 0]).transpose(0,
                                                                                            1)  # Add positional embedding
                x_v = x_v + self.embed_positions(x_in_v.transpose(0, 1)[:, :, 0]).transpose(0,
                                                                                            1)  # Add positional embedding
