        ...
        while True:
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    loss_A_mix = 0.0
                    audio_mix, targets_a, targets_b, targets_a_m, targets_b_m, lam = mixup_data_no_grad(audio,
                                                                                                        labels['A'],
                                                                                                        labels['M'])
                    audio_mix, targets_a, targets_b = map(Variable, (audio_mix, targets_a, targets_b))
                    outputs_A_mix = model(text, audio_mix, vision)
                    fusion_mix, targets_a_m, targets_b_m = map(Variable,
                                                               (outputs_A_mix['Feature_f'], targets_a_m, targets_b_m))
                    loss_A_mix += self.args.A * mixup_criterion(self.criterion, outputs_A_mix['A'], targets_a,
                                                                targets_b, lam).squeeze()
                    loss_A_mix += self.args.M * mixup_criterion(self.criterion, outputs_A_mix['M'], targets_a_m,
                                                                targets_b_m, lam).squeeze()
                    for m in 'TV':
                        loss_A_mix += eval('self.args.' + m) * self.criterion(outputs_A_mix[m], labels[m])

                    loss_V_mix = 0.0
                    vision_mix, targets_a2, targets_b2, targets_a2_m, targets_b2_m, lam2 = mixup_data_no_grad(vision,
                                                                                                              labels[
                                                                                                                  'V'],
                                                                                                              labels[
                                                                                                                  'M'])
                    ...
                    fusion_mix2, targets_a2_m, targets_b2_m = map(Variable, (
                        outputs_V_mix['Feature_f'], targets_a2_m, targets_b2_m))
                    loss_V_mix += self.args.V * mixup_criterion(self.criterion, outputs_V_mix['V'], targets_a2,
                                                                targets_b2, lam2).squeeze()
                    loss_V_mix += self.args.M * mixup_criterion(self.criterion, outputs_V_mix['M'], targets_a2_m,
                                                                targets_b2_m, lam2).squeeze()
                    for m in 'TA':
